...
class GrayCodeDecoder:
    ...
    def decode(self, threshold: float | int) -> tuple[np.ndarray, np.ndarray]:
        ...
        files_to_decode = 4 * self.depth
        for i in range(0, files_to_decode, 2):
            print(f"\rDecoding {i // 2 + 1}/{files_to_decode // 2}", end="")
            current_projection_area = np.zeros(
                self.shape, dtype=np.bool
            )  # bool matrix representing the pixels that are in the projection area for the current greycode

            imgVal1: np.ndarray = patterns[i][
                projection_area
            ]  # pixel values in projection area from first image
            imgVal2: np.ndarray = patterns[i + 1][
                projection_area
            ]  # pixel values in projection area from second image

            diff = cv2.absdiff(
                imgVal1, imgVal2
            )  # absolute difference between the projection area's of pattern and its inverse

            if (
                imgVal1.shape[0] < diff.shape[0]
            ):  # counter absdiff behavior when original shape is (2,) or smaller where it pads unneeded 0's
            ...
            current_projection_area[projection_area] = mask[
                :, 0
            ]  # insert into bool matrix where absdiff is above threshold
            ...
            unknown_pixels = np.zeros(
                self.shape, dtype=np.bool
            )  # bool matrix representing pixels whose greycode is unknown, and are thus invalid
            ...
            unknown_pixels[projection_area] = mask[
                :, 0
            ]  # insert into bool matrix where absdiff is lower than threshold
            result[unknown_pixels] = (
                0  # set codes to zero because bit in current pattern was unknown, making whole code for that pixel unknown and thus invalid
            )

            projection_area[unknown_pixels] = (
                False  # set all invalid pixels in projection area to false
            )
        print()
    ...
    def __get_projection_indices(
        self, full: list[np.ndarray], threshold: float | int
    ) -> tuple[np.ndarray]:
